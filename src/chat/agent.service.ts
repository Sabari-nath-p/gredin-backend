...
// Minimal DB schema description for the LLM prompt (saves tokens)
...
- Keep the answer focused and under 500 words.
  ...
  // ─────────── XML Parser ───────────
  private parseDecision(xml: string): AgentDecision {
    try {
      // Extract action
      ...
      // Fallback: if no XML tags, treat entire response as direct answer
